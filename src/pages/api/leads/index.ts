...
import { isDisposableEmail, escapeHtml } from '../../../lib/email-guards';
...
// inquiry_type whitelist — never pass raw client strings downstream (DB enrichment, AI prompt)
const VALID_INQUIRY_TYPES = ['distributor', 'product_info'] as const;
function sanitizeInquiryType(raw: unknown): string | undefined {
  if (typeof raw !== 'string' || !raw) return undefined;
  return (VALID_INQUIRY_TYPES as readonly string[]).includes(raw) ? raw : 'product_info';
}
...
// IP-based rate limiting for lead submissions — KV-backed (in-memory Maps don't
// survive across Workers isolates, so they never actually limited anything).
...
async function checkLeadRateLimit(
  kv: KVNamespace | undefined,
  ip: string,
): Promise<{ allowed: boolean; retryAfter?: number }> {
  if (!kv) return { allowed: true }; // fail-open if KV unavailable
  try {
    ...
    if (!state || typeof state.windowStart !== 'number' || now - state.windowStart >= LEAD_RATE_LIMIT.windowMs) {
      state = { count: 1, windowStart: now };
    } else {
      state.count = (state.count || 0) + 1;
    }
    if (state.count > LEAD_RATE_LIMIT.maxPerMinute) {
      const retryAfter = Math.max(1, Math.ceil((state.windowStart + LEAD_RATE_LIMIT.windowMs - now) / 1000));
      return { allowed: false, retryAfter };
    }
    await kv.put(key, JSON.stringify(state), { expirationTtl: LEAD_RATE_LIMIT.kvTtlSeconds });
    return { allowed: true };
  } catch (e) {
    // Fail-open: rate limiting must never block legitimate leads
    console.error('[Leads API] KV rate limit check failed (fail-open):', e);
    return { allowed: true };
  }
}
...
export const POST: APIRoute = async ({ request, locals }) => {
  // Rate limiting (KV-backed, fail-open)
  const clientIP = request.headers.get('cf-connecting-ip')
    || request.headers.get('x-forwarded-for')?.split(',')[0]?.trim()
    || 'unknown';

  const envEarly = (locals as any).runtime?.env as Env | undefined;
  const rateCheck = await checkLeadRateLimit(envEarly?.SESSION, clientIP);
  ...
  try {
    ...
    // 1b. Reject disposable/temporary emails server-side (client check is UX only)
    ...
    // Whitelist inquiry_type — anything unknown coerces to 'product_info'
    const inquiryType = sanitizeInquiryType(data.inquiry_type);
    ...
    if (inquiryType) enrichment.inquiry_type = inquiryType;
    ...
    // leads.email is UNIQUE — on conflict this is a RETURNING PROSPECT, not an error:
    // merge the new inquiry into the existing lead instead of dropping it.
    ...
    try {
      ...
      if (!insertErr?.message?.includes('UNIQUE constraint failed')) throw insertErr;

      // Repeat inquiry: update the existing lead in place
      const existing = await db.prepare(
        'SELECT id, message, interested_products, enrichment_data, status FROM leads WHERE email = ?'
      ...
      if (!existing) throw insertErr; // race: row vanished — treat as real failure

      isRepeatInquiry = true;
      leadId = existing.id;

      // Append new message with a timestamp separator
      const stamp = new Date().toISOString();
      const newMessage = data.message || '(no message)';
      const mergedMessage = existing.message
        ? `${existing.message}\n\n--- REPEAT INQUIRY ${stamp} ---\n${newMessage}`
        : `--- REPEAT INQUIRY ${stamp} ---\n${newMessage}`;

      // Merge interested_products (union, preserving order)
      let prevProducts: string[] = [];
      try {
        const parsed = JSON.parse(existing.interested_products || '[]');
        if (Array.isArray(parsed)) prevProducts = parsed;
      } catch { /* keep empty */ }
      const mergedProducts = [...new Set([...prevProducts, ...interestedProducts])];

      // Merge enrichment_data: update inquiry_type (+ referral_source) with latest values
      let prevEnrichment: Record<string, unknown> = {};
      try {
        const parsed = JSON.parse(existing.enrichment_data || '{}');
        if (parsed && typeof parsed === 'object') prevEnrichment = parsed;
      } catch { /* keep empty */ }
      const mergedEnrichment = { ...prevEnrichment, ...enrichment };

      // Re-open dead leads: 'lost' is the closed/dead status in this pipeline
      // (status values: new/contacted/qualified/proposal/won/lost)
      const newStatus = existing.status === 'lost' ? 'new' : existing.status;
      ...
      `).bind(
        ...
        Object.keys(mergedEnrichment).length ? JSON.stringify(mergedEnrichment) : null,
        newStatus,
        existing.id,
      ).run();
    }

    const repeatTag = isRepeatInquiry ? '[REPEAT INQUIRY] ' : '';
    console.log(
      `[Leads API] Lead ${isRepeatInquiry ? 'updated (repeat inquiry)' : 'created'}:`,
      leadId, `Grade ${initialScoring.grade}, Score ${initialScoring.total}`,
    );

    // Log activity (non-blocking)
    logActivity(db, {
      type: isRepeatInquiry ? 'lead_updated' : 'lead_created',
      detail: `${repeatTag}${isRepeatInquiry ? 'Returning lead' : 'New lead'}: ${data.company_name} (${data.country}) — Grade ${initialScoring.grade}, Score ${initialScoring.total}${isFree ? ' [Free email]' : ''}`,
      ip: clientIP,
    }).catch(() => {});
    ...
    try {
      await notifyNewLead(env, {
        ...
        company: `${repeatTag}${data.company_name || ''}`.trim() || undefined,
        name: data.contact_name,
        email: data.email,
        country: data.country,
        product_interest: interestedProducts.join(', '),
        message: isRepeatInquiry ? `[REPEAT INQUIRY] ${data.message || '(no message)'}` : data.message,
    ...
    if (slackUrl) {
      const slackPayload = {
        companyName: `${repeatTag}${data.company_name || 'N/A'}`,
    ...
    if (resendKey && data.email && source === 'website') {
      const contactName = escapeHtml(data.contact_name || 'there');
      const products = escapeHtml(interestedProducts.join(', '));
      const confirmHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="border: 1px solid #e2e8f0; border-top: 0; padding: 24px; border-radius: 0 0 12px 12px;">
            ...
            <div style="background: #f8fafc; border-radius: 8px; padding: 16px; margin: 16px 0;">
              <p style="color: #64748b; font-size: 14px; margin: 0 0 8px;"><strong>Company:</strong> ${escapeHtml(data.company_name || 'N/A')}</p>
              ${products ? `<p style="color: #64748b; font-size: 14px; margin: 0 0 8px;"><strong>Products of Interest:</strong> ${products}</p>` : ''}
              <p style="color: #64748b; font-size: 14px; margin: 0;"><strong>Country:</strong> ${escapeHtml(data.country || 'N/A')}</p>
            </div>
    ...
    if (ctx?.waitUntil && env?.ANTHROPIC_API_KEY && env?.GMAIL_CLIENT_ID && source === 'website') {
      ...
      // Whitelisted value only — never interpolate raw client strings into the prompt
      const inquiryTypeForPrompt = inquiryType || '(not specified)';
...
- Inquiry type (their selected intent): ${inquiryTypeForPrompt}  (distributor = wants a country distributorship; product_info = wants product information / where to buy)
...
sh.lee@britzmedi.com`,
          ...
          try {
            await createGmailDraft(
              ...
              },
              {
                ...
                htmlBody: `<div style="font-family: Arial, sans-serif; line-height: 1.7; white-space: pre-line;">${escapeHtml(draftBody)}</div>`,
              },
            );
            gmailOk = true;
  ...
  <div style="background: #f8fafc; border-radius: 8px; padding: 16px; margin-bottom: 16px;">
    <p style="margin: 4px 0; color: #475569;"><strong>To:</strong> ${escapeHtml(data.contact_name)} &lt;${escapeHtml(data.email)}&gt;</p>
    <p style="margin: 4px 0; color: #475569;"><strong>Company:</strong> ${escapeHtml(data.company_name)}</p>
    <p style="margin: 4px 0; color: #475569;"><strong>Products:</strong> ${escapeHtml(interestedProducts.join(', '))}</p>
  </div>
  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 24px; white-space: pre-line; line-height: 1.7; color: #1e293b; font-size: 15px;">${escapeHtml(draftBody) || '(AI draft generation failed)'}</div>
</div>`,
    ...
    return new Response(JSON.stringify({
      success: true,
      repeat: isRepeatInquiry,
      lead: { id: leadId, ...data, lead_score: initialScoring.total, lead_grade: initialScoring.grade },
    }), {
      status: isRepeatInquiry ? 200 : 201,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error: any) {
    console.error('[Leads API] Error creating lead:', error);

    // Never lose lead data silently: best-effort fallback notification to admin
    // with the raw payload, so the lead can be followed up manually.
    try {
      ...
      if (resendKey && data?.email) {
        await sendEmail({
          apiKey: resendKey,
          to: 'sh.lee@britzmedi.com',
          subject: `[LEAD SAVE FAILED] ${data.company_name || data.email}`,
          html: `<div style="font-family: Arial, sans-serif; max-width: 700px; padding: 20px;">
  <p style="font-size: 15px; color: #b91c1c; margin: 0 0 16px;"><strong>Saving this lead to the database failed.</strong> Raw submission below — please follow up manually.</p>
  <p style="font-size: 13px; color: #64748b; margin: 0 0 16px;">Error: ${escapeHtml(error?.message || 'unknown error')}</p>
  <pre style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px; font-size: 13px; white-space: pre-wrap; word-break: break-word;">${escapeHtml(JSON.stringify(data, null, 2))}</pre>
</div>`,
          from: 'BRITZMEDI Global <noreply@britzmedi.com>',
        });
        console.log('[Leads API] Fallback lead payload emailed to admin');
      }
    } catch (notifyErr) {
      console.error('[Leads API] Fallback lead notification failed:', notifyErr);
    }
